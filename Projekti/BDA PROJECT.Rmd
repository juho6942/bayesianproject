...
if (F){
  install.packages("rstudioapi") #BRMS parallelization
}
library(car)  #VHI
library(rstudioapi) #parallel chains in brms
...
newcsv = subset(data, select= -c(BloodCulture,bacteremia))
newcsv = subset(newcsv,select=c("CRP","WBC","NEU","LYM","PLT","CREA","ALB","BUN","GBIL","RBC","SODIUM","SEX","AGE","bact_num"))
newcsv = drop_na(newcsv)
...
d=getData(1)
cols = c("CRP","WBC","NEU","LYM","PLT","CREA","ALB","BUN","GBIL","RBC","SODIUM")
sd = vector(mode="numeric",len =  length(cols))
mu = vector(mode="numeric",len =  length(cols))
...
for (c in 1:length(cols)) {
  ...
  #pdf(file=paste(c,".pdf"))
  #sample_quant = quantile(d[[cols[c]]],quants)
  #norm_quant = qnorm(quants, sd = sd[c], mean = mu[c])
  #student_quant = qt(quants, df = 3) * sd[c] + mu[c]
  #exp_quant = qexp(quants, rate = 1 / mu[c])
  
  #data = d[[cols[c]]][d[[cols[c]]]>0]  %>% log() %>% sort()
  data=d[[c]] 
  #normalized = ((data-mean(data))/sd(data))  
  # Generate the plots directly in the Plots tab
  
  #b = seq(min(data),max(data),len=80)

  #hist(data %>% sort(),breaks=b,main=paste("hist ", cols[c]))
  plot(density(data), main=cols[c])
  curve(dnorm(x), add=T,col="red")
  
  
  #ABLine aivan vituillaan
  #qqPlot(d[[cols[c]]],distribution="norm",ylab=cols[c])
  #qqPlot(d[[cols[c]]],distribution="t",df=3,ylab=cols[c])
  #qqPlot(d[[cols[c]]],distribution="exp",ylab=cols[c])
  #dev.off()

}
for (c in c("RBC","ALB")) {
  d[c] = (d[[c]]-mean(d[[c]]))/sd(d[[c]])
  
}
plot(density(d[["CRP"]]))
for (c in c("CRP","WBC","NEU","LYM","PLT","CREA","BUN","GBIL","SODIUM")) {
  col = d[[c]]
  minim = min(col[col>0])
  ...
  print(sum(col==0))
  logged = col %>% log() 
  lognorm = (logged - mean(logged))/sd(logged)
  d[c] = lognorm
  
}

# write.csv(d, file("standardized_data.csv"))


###Tjoo Jatkan tästä huomenna, mutta ideana että ei voi vaan skaalata suoraan vaan pitää ettiä sopiva jakauma varmaankin
# Example: decades with enough n per band

d$age_br <- cut(d$AGE, breaks = c(0,30,60,Inf),
                right = FALSE, include.lowest = TRUE)


```
Sodium voisi olla student-t
RCB saleen normi
...
getData <- function(x){
  # current best data
  if (x==1)
    d =read.csv("standardized_data.csv")
...
```
Täytyy keksiä joku fiksumpi tapa kirjottaa noita micitettyjä datoja. Ehkä erillisille CSV? 
Factors? Näillä saa hierarkian rakennettua(?)
...
f_pooled = as.formula(" bact_num ~ 1 + CRP+ WBC+ NEU+ LYM + PLT+ CREA+ ALB+ BUN+ GBIL+ SODIUM+ RBC")
f_sex = as.formula(" bact_num ~ 1 + CRP+ WBC+ NEU+ LYM + PLT + ALB+ BUN+ GBIL+ SODIUM+ (RBC+CREA|SEX)")
f_age = as.formula(" bact_num ~ 1 + CRP+ WBC+ NEU+ LYM + PLT  + GBIL+ SODIUM+ (ALB+ BUN+RBC+CREA|age_br)")
f_agesex = as.formula(" bact_num ~ 1 + CRP+ WBC+ NEU+ LYM + PLT  + GBIL+ SODIUM+ (ALB+BUN|age_br)+(RBC+CREA|SEX)")
...
```{r}
##### PRIORS #####
#pooled_prior = set_prior("normal(0,1)",class='b')
#intercept_prior = set_prior("normal(0,5)",class="intercept")

```
Nää pitäis varmaan skaalata jotenkin 
https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations
Max_treedepth ongelmia tutkiessani törmäsin ketjuun missä kaverilla oli vähän isompi malli (2 leveliä hierarkiaa, 25 variablea, 50k.) Fitti kesti 96h :DD

Pitäskö tän olla bernoulli? 
```{r}
##### MODEL ######

#pooled_m = brm(formula  =f_pooled,
#               family="bernoulli",
#               data=d,
#               prior=pooled_prior,
#               cores=4)

sex_m =brm(formula  =f_sex,
               family="bernoulli",
               data=d,
               
               cores=4)
age_m = brm(formula  =f_age,
               family="bernoulli",
               data=d,
               
               cores=4)

agesex_m = brm(formula  =f_agesex,
               family="bernoulli",
               data=d,
               cores=4)
