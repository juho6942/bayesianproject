...
for (c in 1:length(cols)) {
  ...
  plot(sort(d[[c]]),main=cols[c])
  #qqplot(norm_quant, sample_quant, main = paste("normal ", cols[c]));abline(norm_quant[100],norm_quant[900],col="red")
  
  #qqplot(student_quant, sample_quant, main = paste("T ",cols[c]));abline(student_quant[100],student_quant[900],col="red")
  #qqplot(exp_quant, sample_quant, main = paste("EXP ",cols[c]));abline(exp_quant[100],exp_quant[900],col="red")
...
f_pooled = as.formula("bact_num ~ 1 + MCV + HGB + PLT + MCHC + RDW + MPV + 
    ...
    TRIG + CHOL + CRP + BASOR + EOSR + MONOR + NEUR + PDW + RBC + 
    WBC")
#drop id sex age
#
f_pooled = as.formula(" bact_num ~ 1 + CRP+ WBC+ NEUR+ PLT+ CREA+ ALB+ BUN+ GBIL+ SODIUM+ RBC")
...
intercept_prior = set_prior("normal(0,1)",class="intercept")
